...
// ─── Utility: build correct backend origin from env var ──────────────────────
// REACT_APP_API_URL should be the backend root, e.g.:
//   dev:  http://localhost:5000
//   prod: https://chatapp-vj1t.onrender.com
// Never set it to the Vercel frontend URL on the backend service.
const getBackendOrigin = () => {
  const raw = (process.env.REACT_APP_API_URL || "http://localhost:5000").replace(/\/$/, "");
  // Strip accidental /api suffix so we always work with the plain origin
  return raw.endsWith("/api") ? raw.slice(0, -4) : raw;
};

const ChatBubble = ({ text, align = "left", delay = 0, color = "rgba(255,255,255,0.08)" }) => (
  <Box sx={{ display:"flex", justifyContent: align === "right" ? "flex-end" : "flex-start", animation:`floatBubble 4s ease-in-out ${delay}s infinite` }}>
    <Box sx={{ background:color, border:"1px solid rgba(255,255,255,0.1)", borderRadius: align === "right" ? "18px 18px 4px 18px" : "18px 18px 18px 4px", px:2, py:1, maxWidth:"75%" }}>
...
const LoginPage = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // The login response may return `result.user` directly (best case),
      // or just a token. We fall back to decoding the token for basic fields,
      // but theme will only be present if the backend returns the full user.
      let user = result.user;
      if (!user) {
        try {
          const decoded = jwtDecode(result.token);
          user = { _id: decoded.id, id: decoded.id, name: decoded.name };
        } catch {
          user = {};
        }
      }
      ...
      // socket has autoConnect:false so we connect explicitly here.
      // This ensures the user is marked online as soon as they log in,
      // and the backend 'setup' event receives the correct user object.
  ...
  return (
    <>
      ...
      <Box sx={{ minHeight:"100vh", display:"flex", fontFamily:"'Sora', sans-serif", background:"#f8fafc" }}>
        ...
        {/* ── LEFT: Form Panel ── */}
        <Box sx={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", p:{ xs:3, sm:6 }, position:"relative", background:"#f8fafc", order:{ xs:1, md:0 } }}>
          ...
          <Box sx={{ width:"100%", maxWidth:420, position:"relative", zIndex:1, animation:"slideInLeft 0.7s ease forwards" }}>
            ...
            {/* Google button */}
            <Button variant="outlined" fullWidth startIcon={<GoogleIcon sx={{ fontSize:"1.1rem !important" }} />} sx={{ borderRadius:"12px", py:1.4, fontFamily:"'Sora', sans-serif", fontWeight:500, fontSize:"0.875rem", borderColor:"#e2e8f0", color:"#334155", background:"#fff", boxShadow:"0 1px 3px rgba(0,0,0,0.06)", mb:3, "&:hover":{ borderColor:"#cbd5e1", background:"#f8fafc", boxShadow:"0 2px 8px rgba(0,0,0,0.08)" } }}>
            ...
            <Stack spacing={2.5}>
              <TextField
                ...
                InputProps={{ startAdornment: <InputAdornment position="start"><EmailIcon sx={{ fontSize:"1.1rem", opacity:0.4, color:"#334155" }} /></InputAdornment> }}
                sx={inputSx("email")}
              />
              ...
              <TextField
                ...
                InputProps={{
                  startAdornment: <InputAdornment position="start"><LockOutlinedIcon sx={{ fontSize:"1.1rem", opacity:0.4, color:"#334155" }} /></InputAdornment>,
                  endAdornment: (
                    <InputAdornment position="end">
                      <IconButton onClick={() => setShowPassword(p => !p)} edge="end" sx={{ color:"#94a3b8", mr:-0.5 }}>
              ...
              <Box sx={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                <FormControlLabel
                  control={<Checkbox size="small" checked={rememberMe} onChange={e => setRememberMe(e.target.checked)} sx={{ color:"#cbd5e1", "&.Mui-checked":{ color:ACCENT }, p:0.5 }} />}
                  label={<Typography sx={{ fontSize:"0.825rem", color:"#64748b", fontFamily:"'Sora', sans-serif" }}>Remember me</Typography>}
                />
                <Typography sx={{ fontSize:"0.825rem", color:ACCENT, cursor:"pointer", fontWeight:500, fontFamily:"'Sora', sans-serif", "&:hover":{ textDecoration:"underline" } }}>
                  Forgot password?
                </Typography>
              </Box>
              ...
              <Button
                ...
                onClick={handleLogin}
                sx={{ mt:0.5, borderRadius:"12px", py:1.6, fontSize:"0.9rem", fontWeight:600, fontFamily:"'Sora', sans-serif", background:`linear-gradient(135deg, ${ACCENT} 0%, #8b5cf6 100%)`, boxShadow:`0 4px 20px rgba(99,102,241,0.3)`, transition:"all 0.25s ease", letterSpacing:"0.01em", "&:hover":{ transform:"translateY(-2px)", boxShadow:`0 8px 30px rgba(99,102,241,0.4)` }, "&:active":{ transform:"translateY(0)" }, "&:disabled":{ background:"#e2e8f0", color:"#94a3b8", boxShadow:"none" } }}
              >
                {loading ? (
            ...
            <Box sx={{ display:"flex", gap:3, mt:5, pt:4, borderTop:"1px solid #f1f5f9" }}>
              {[{ num:"50K+", label:"Active users" }, { num:"99.9%", label:"Uptime" }, { num:"4.9★", label:"App rating" }].map(s => (
                <Box key={s.label}>
        ...
        <Box sx={{ display:{ xs:"none", md:"flex" }, flexDirection:"column", justifyContent:"space-between", width:"44%", minHeight:"100vh", background:"linear-gradient(145deg, #1e1b4b 0%, #0f172a 40%, #0c1a2e 100%)", backgroundSize:"200% 200%", animation:"gradientShift 10s ease infinite", p:6, position:"relative", overflow:"hidden", order:1 }}>
